...
const ProfilePage = () => {
  ...
  const handleEditProduct = async (productId) => {
    try {
      ...
      alert("Cannot udate product:", err.message);
      ...
      <div className="flex flex-wrap gap-6 justify-center">
        {products.map((elem) => {
          return (
            <div
              key={elem._id}
              className="p-5 px-20 rounded-lg border-1 mt-4 min-w-70 bg-amber-100"
            >
              <p>{elem.title}</p>
              {elem._id === editProductId ? (
                <>
                  <input
                    value={updatedPrice}
                    onChange={(e) => seUpdatedPrice(e.target.value)}
                  ...
                  <button
                    ...
                    }} className="py-0.5 px-2 border-1 rounded-md min-w-23"
                  >
                    Update
                  </button>
                </>
